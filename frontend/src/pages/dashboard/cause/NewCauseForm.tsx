...
export default function NewCauseForm({ defaultCauseType = 'demand', onSuccess }: { defaultCauseType?: 'demand' | 'support', onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else navigate('/dashboard?tab=Mobilise');
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-6 border border-gray-300 rounded-2xl  font-poppins">
      {/* <h1 className="text-2xl font-bold mb-6">Create New Cause</h1> */}
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              ...
              {(statesLGAWardList.find(s => s.state === form.location.state)?.lgas || []).map((lgaObj) => (
                <option key={lgaObj.lga} value={lgaObj.lga}>{lgaObj.lga.replace(/-/g, ' ')}</option>
              ))}
            </select>
            <select
              name="ward"
            ...
            >
              ...
              {(statesLGAWardList.find(s => s.state === form.location.state)?.lgas.find(l => l.lga === form.location.lga)?.wards || []).map((ward) => (
                <option key={ward} value={ward}>{ward.replace(/-/g, ' ')}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div>
          <label className="block mb-1 font-medium">{defaultCauseType === 'support' ? 'Decision Makers' : 'Target Candidates'}</label>
          <div className="flex flex-wrap gap-2 mb-2">
          ...
          <input value={targetInput} onChange={(e) => setTargetInput(e.target.value)} onKeyDown={(e) => handleTagInputKeyDown(e, 'targets', targetInput, setTargetInput)} placeholder={`Add ${defaultCauseType === 'support' ? 'decision maker' : 'candidate'} (press comma or Enter)`} className="w-full px-4 py-2 border rounded" />
        </div>
