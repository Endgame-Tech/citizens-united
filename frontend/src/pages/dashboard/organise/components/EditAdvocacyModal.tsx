...
import { updateAdvocacy } from '../../../../services/advocacyService';
...
export default function EditAdvocacyModal({ isOpen, advocacy, onClose, onSuccess }: EditProps) {
  ...
  const [toolkitInput, setToolkitInput] = useState<Toolkit>({
    label: '',
    url: '',
    type: 'Toolkit',
  });
  ...
  useEffect(() => {
    if (advocacy) {
      setForm({
        ...
        goals: advocacy.goals || [],
        toolkits: advocacy.toolkits || [],
        displayImage: advocacy.displayImage || '',
      });
    }
  }, [advocacy]);
  ...
  const uploadCroppedImage = async () => {
    ...
    try {
      ...
      setFileSrc(null);
      setToastInfo({ message: 'Image uploaded successfully', type: 'success' });
    } catch (err) {
      setToastInfo({ message: 'Image upload failed', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const updated = await updateAdvocacy(advocacy._id, form);
      setToastInfo({ message: 'Advocacy updated successfully', type: 'success' });
      onSuccess(updated);
      onClose();
    } catch (err) {
      console.error(err);
      setToastInfo({ message: 'Failed to update advocacy', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => onClose()}>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">Edit Advocacy</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input name="title" value={form.title} onChange={handleChange} required className="w-full px-4 py-2 border rounded-lg" />
          </div>

          <div>
            <label className="block mb-1 font-medium">Description</label>
            <textarea name="description" value={form.description} onChange={handleChange} required className="w-full px-4 py-2 border rounded-lg" rows={4} />
          </div>
          ...
          <div>
            ...
            <input value={goalInput} onChange={(e) => setGoalInput(e.target.value)} onKeyDown={handleGoalKeyDown} className="w-full px-4 py-2 border rounded-lg" />
          </div>
