...
import type { EvaluationCategory } from "../../../../../../types/evaluationQuestions";


interface CapacitySectionProps {
  onNext: (data: { capacityScore: number }) => void;
}

const CapacitySection: React.FC<CapacitySectionProps> = ({ onNext }) => {
  const { title, sections }: EvaluationCategory = evaluationQuestions.capacity;
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full bg-white rounded-xl p-5 sm:p-6 md:p-8 shadow-sm border border-gray-100">
      ...
      <div className="text-sm text-gray-600 mb-6 bg-blue-50 p-3 rounded-lg">
        <p>Evaluate the candidate's capacity to govern effectively and manage resources.</p>
      </div>
      ...
      <div className="mt-6 flex justify-end">
        <button
          type="submit"
          className="px-6 py-2.5 bg-[#006837] text-white rounded-lg hover:bg-[#00592e] transition-colors duration-200 text-sm font-medium min-w-[120px]"
        >
          Continue
        </button>
      </div>
